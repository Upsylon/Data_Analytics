...
```{r}
plot_correlation(cred, type="all", title = "Correlation Graph") # Attention! Il faut seulement ploter avec les valeurs continues. Les catégoriques se feront avec le khi-2! Test de wilkockson pour tester si les groupes sont différents.
```
...
Before beginning to work on our different models, we can create a test set and a training set in order to build the models. This procedure is used is order to avoid overfitting and to predict instances which have to be used to build the model. 
...
```{r}
set.seed(111) # a seed (the number is the source of our imagination!)
...
NOTE: c'est pas vraiment une cross-validation vu qu'on utilise le MEME test et training set, mais le principe est le même. On est en train de faire le "tuning" du modèle et la CV sera faite plus tard.
...
sd.acc.nnet.cv[-c(1:10)] # average sd for the models with i = 11,..,30 nodes
accu <- data.frame(Average=mean.acc.nnet.cv[-c(1:10)], Sd = sd.acc.nnet.cv[-c(1:10)], Neurones = c(11:30))
```

Using the *1 - SE rule*, it is possible to compare our neural network with different numbers of neurones in order to see which ones are considered to be equivalent. 

```{r, comment=NA}
mean.acc.nnet.cv[which.max(mean.acc.nnet.cv)] - sd.acc.nnet.cv[which.max(mean.acc.nnet.cv)]
```

The maximum accuracy to which we substract its standard deviation is equal to `r mean.acc.nnet.cv[which.max(mean.acc.nnet.cv)] - sd.acc.nnet.cv[which.max(mean.acc.nnet.cv)]`
. All models with an accuracy more than this value can be considered.  
On the following plot, we can see at a glance which models can to be considered and that are above the red line representing the model with maximum average accuracy to which we substracted its standard deviation.


```{r, echo=FALSE}
accu %>% ggplot() + 
  geom_point(aes(x = Neurones, y = Average)) +
  geom_hline(yintercept = mean.acc.nnet.cv[which.max(mean.acc.nnet.cv)] - sd.acc.nnet.cv[which.max(mean.acc.nnet.cv)], col='red') + 
  scale_x_continuous(breaks = c(11:30)) + ylab("Average accuracy") + ggtitle("Average accuracy according to the number of neurones in our network") + my_theme()
```

<!-- We can directly see which models satisfy the rule, and the simplest one should be considered in this case (to reduce overfitting). This model is the model with `r which(mean.acc.nnet.cv==mean.acc.nnet.cv[mean.acc.nnet.cv>mean.acc.nnet.cv[which.max(mean.acc.nnet.cv)] - sd.acc.nnet.cv[which.max(mean.acc.nnet.cv)]][1])` neurones.   -->
In addition, based on some [litterature](https://stats.stackexchange.com/questions/181/how-to-choose-the-number-of-hidden-layers-and-nodes-in-a-feedforward-neural-netw), one can see that a rule of thumb proposes to take a number of neurones that is the average between our input nodes (30) and outputs (1 nodes with two level), which should therefore be 16 neurones.  
As the model linked with this number of neurones satisfies and the rule of thumb and the *1 - SE* rule, we decide to choose this model no fit our neural network.  
...
We can therefore continue to explore further the model with 16 neurones that we retained. 

### Model selection and predicting the values of the testing set

We present you the characteristics of the model retained, with 16 neurons in the hidden layer.  
...
nnet.model.retained <- nnet(cred.train$response ~ .,data = cred.train,maxit = 200,size = 16, decay = 1)

pred.nnet.retained <- ifelse(predict(nnet.model.retained, cred.test) > 0.5, "good",  "bad") # predictions on the test set
...
In addition, we can plot our model. It can be quite complex, there a plenty of arrows. It is not necessary to understand precisely all of them and they can be seen as a "black box", meaning that the interpretability of such a model is modest. But, afterall, what we need is to make good predictions!
...
We obtain a final accuracy of `r acc.nnet.retained` on our testing set.
