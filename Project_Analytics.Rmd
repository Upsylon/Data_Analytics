...
# nnet.model = list()
# for (i in 1:30) {
#   nnet.model[[i]] = nnet(as.factor(cred.train$response)~., data = cred.train, size=i, maxit=200, decay = 1)
# } 
# 
...
#   pred.nnet[[i]] <- ifelse(pred.nnet[[i]]  > 0.5 , 1,  0)
# } 
# 
...
for (j in 2 ^ (-6:0)) {
  ...
  for (i in 1:length(cred)) {
    nnet.model[[10 * i + k]] <- nnet(as.factor(cred.train$response) ~ ., data = cred.train,  maxit = 200,
                                    size = i,
                                    decay = j
    )
  }
...
We present you the characteristics of the model with `r which.max(acc.nnet)` neurons in the hidden layer and that proposes the highest accuracy on the testing set among all.
...
We obtain a final accuracy of `r acc.nnet[[which.max(acc.nnet)]]` on our testing set.
