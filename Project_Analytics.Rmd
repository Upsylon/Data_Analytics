...
The selected knn model uses a distance of 2 and is computed with the `r knn_fit$results[which.max(knn_fit$results$Accuracy),]$k` neerest neighbors. The obtained accuracy using a 10 fold Cross-Validation is equal to `r max(knn_fit$results$Accuracy)`. 
