...
The second ensemble method that we consider in bagging (which stands for Bootstrap AGGregatING). Again, the goal of using this technique is to increase the predictive power of the learners. It consists of drawing several random samples (with replacement) from the training data set, to construct a separate model for each sample and separate predictions for the test set. Finally, one should averaged these predictions to create a final prediction value.
