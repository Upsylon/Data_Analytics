...
nnet.model.retained <- 
  nnet(cred.train$response ~ .,
       ...
       size = which.max(mean.acc.nnet.cv), 
       decay = round(seq(2^-8,2,2^-3)[which.max(mean.acc.nnet.cv2)],1))
...
paste("The selected knn model uses a distance of", selected.knn$distance) %>% 
  paste("and is computed with the", ifelse(which.max(acc.knn) < 20,
                                           which.max(acc.knn), 
                                           which.max(acc.knn) - 20)) %>% 
  paste("neerest neighbors.") %>%
  paste("The obtained accuracy is equal to", 
        acc.knn[[which.max(acc.knn)]]) 

# confusion matrix
pred.knn[which.max(acc.knn)] %>% 
  kable(caption = "Confusion matrix of the fitted KNN model",
        col.names = c("Predict good", "Predict bad")) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    full_width = F,
...
for (i in 1:10){
  ...
  glm.cv <- glm(response ~., 
   ...
   # the tuned (stepwise) logistic regression 
  logit.step.cv <- step(glm.cv, direction="both")
  
  # The predictions on the test set i based on the stepwise regression
  glm.pred.cv <- ifelse(
  predict(logit.step.cv, newdata=test.list[[i]], type="response") > 0.5, 
  "good", "bad"
  ) 
  
  # the confusion matrix
  tab.glm.cv <- table(test.list[[i]]$response, glm.pred.cv) 
  
  # The final accuracy
  acc.glm.cv[i] <- accuracy(tab.glm.cv) 
}
